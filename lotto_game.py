...
class Lotto:
    def __init__(self):
        ...
        self.submit = Button(root,text="Check validation",command=login).place(x=170, y=230)

        # time label
        now = dt.datetime.now()
        self.time = Label(root)
        self.time.place(x=150, y=280)
        self.time.config(text="Date: " + str(now.strftime("%Y-%m-%d %H:%M:%S")))

        # entry
        self.var1 = StringVar()
        self.var2 = StringVar()
        self.user_name_entry = Entry(root,textvariable=self.var1).place(x=290, y=50)
        self.user_surname_entry = Entry(root,textvariable=self.var2).place(x=290, y=100)
